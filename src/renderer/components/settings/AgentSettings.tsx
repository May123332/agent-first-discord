...
export function AgentSettings({ settings }: { settings: TSettings; }) {
    ...
    return (
        <Forms.FormSection>
            ...
            <TextInput value={agent.invocationPrefix} onChange={v => (settings.agent = { ...agent, invocationPrefix: v })} />
            ...
            <TextInput value={String(agent.temperature)} onChange={v => (settings.agent = { ...agent, temperature: Number(v) || 0 })} />

            <Forms.FormTitle>Maximum Tokens</Forms.FormTitle>
            <TextInput value={String(agent.maxTokens)} onChange={v => (settings.agent = { ...agent, maxTokens: Number(v) || 1 })} />

            <Forms.FormTitle>Rate Limit / Minute</Forms.FormTitle>
            <TextInput value={String(agent.rateLimitPerMinute)} onChange={v => (settings.agent = { ...agent, rateLimitPerMinute: Number(v) || 1 })} />
            ...
            {agent.mode === "local" && (
                <>
                    ...
                    <TextInput value={agent.localModel} onChange={v => (settings.agent = { ...agent, localModel: v })} />
                </>
            )}
            ...
            {agent.mode === "online" && (
                <>
                    ...
                    <TextInput value={agent.onlineProvider} onChange={v => (settings.agent = { ...agent, onlineProvider: v === "anthropic" ? "anthropic" : "openai" })} />
                    <Forms.FormTitle>Online Model</Forms.FormTitle>
                    <TextInput value={agent.onlineModel} onChange={v => (settings.agent = { ...agent, onlineModel: v })} />
                </>
            )}
